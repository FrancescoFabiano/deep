...
[[nodiscard]]
BeliefFormulaType BeliefFormula::get_formula_type() const noexcept {
  if (m_formula_type == BeliefFormulaType::BF_TYPE_FAIL) {
    ExitHandler::exit_with_message(
        ExitHandler::ExitCode::BeliefFormulaTypeUnset,
        "Error in reading a BeliefFormula "
        "(BeliefFormulaType not set properly).");
  }
...
void BeliefFormula::set_fluent_formula(
  ...
  if (to_set.empty()) {
    ExitHandler::exit_with_message(
        ...
        "Error in declaring a BeliefFormula: "
        "there must be at least one fluent in a formula.");
  }
...
BeliefFormula::get_fluent_formula() const noexcept {
  ...
  if (m_fluent_formula.empty()) {
    ExitHandler::exit_with_message(
        ...
        "Error in reading a BeliefFormula: "
        "the FluentFormula has not been grounded.");
  }
...
[[nodiscard]]
const BeliefFormula &BeliefFormula::get_bf1() const {
  if (!m_bf1) {
    ExitHandler::exit_with_message(
        ExitHandler::ExitCode::BeliefFormulaMissingNested,
        "Error in declaring a BeliefFormula: "
        "a nested belief formula has not been declared.");
  }
...
[[nodiscard]]
const BeliefFormula &BeliefFormula::get_bf2() const {
  if (!m_bf2) {
    ExitHandler::exit_with_message(
        ExitHandler::ExitCode::BeliefFormulaMissingNested,
        "Error in declaring a BeliefFormula: "
        "a second nested belief formula has not been declared.");
  }
...
BeliefFormula::get_operator() const noexcept {
  ...
  if (m_formula_type == BeliefFormulaType::BF_TYPE_FAIL) {
    ExitHandler::exit_with_message(
        ...
        "Error in reading a BeliefFormula "
        "(BeliefFormulaOperator not set properly).");
  }
...
void BeliefFormula::set_group_agents(
  ...
  if (to_set.empty()) {
    ExitHandler::exit_with_message(
        ...
        "Error in declaring a BeliefFormula: "
        "there must be at least one agent for group formulae.");
  }
...
bool BeliefFormula::operator==(
  ...
  case BeliefFormulaType::BF_EMPTY:
    // Formula types were already checked above.
    return true;
...
BeliefFormula::operator=(
   ...
   * unique_ptr members. Without it, assigning a formula to itself
   * could invalidate data that is still being read from to_copy.
   */
  if (this == &to_copy) {
    return *this;
  }
  ...
  /*
   * Clear all type-specific state first.
   *
   * This prevents stale data from surviving assignments such as:
   *
   *   (A AND B) -> TRUE
   *   B_i(phi)  -> fluent
   */
  m_fluent_formula.clear();
  m_group_agents.clear();
  m_bf1.reset();
  m_bf2.reset();
  ...
  case BeliefFormulaType::FALSE_FORMULA:
  case BeliefFormulaType::BF_EMPTY:
    // These formula types contain no additional data.
    break;
...
bool BeliefFormula::operator<(
  ...
  case BeliefFormulaType::PROPOSITIONAL_FORMULA:
    ...
    default:
      ExitHandler::exit_with_message(
          ExitHandler::ExitCode::BeliefFormulaOperatorUnset,
          "Error in comparing BeliefFormula objects.");
  ...
  case BeliefFormulaType::FALSE_FORMULA:
  case BeliefFormulaType::BF_EMPTY:
    // Same type and no additional data => equal for ordering.
    return false;
