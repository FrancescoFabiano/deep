 ...
 * \brief Stores the initial state information, including pointed world and belief conditions.
 *
 * \see domain
...
{
public:
    /// \brief Default constructor. Initializes all members with their default constructors.
    InitialStateInformation() = default;

    /**
     * \brief Adds a condition for the initial pointed world.
     * \param[in] to_add Fluent formula that the initial pointed world must entail.
     */
     ...
     * \return The fluent formula that all initial worlds must entail if S5 is required.
     */
    [[nodiscard]] const FluentFormula& get_ff_forS5() const;

private:
    /// Formula containing all the fluents required by the possible initial pointed world.
