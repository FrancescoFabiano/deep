...
PlankFormulaConverter::PlankFormulaConverter(
    const std::vector<Fluent> &fluents,
    const std::vector<Agent> &agents)
    : m_fluents(fluents),
      m_agents(agents) {}

BeliefFormula PlankFormulaConverter::convert(
    const plank::del::formula_ptr &formula) const {

  return std::visit(
      [this]<typename T0>(
          const T0 &typed_formula) -> BeliefFormula {
        ...
        if constexpr (
          ...
          result.set_formula_type(
              BeliefFormulaType::TRUE_FORMULA);
          ...
          BeliefFormula result;
          result.set_formula_type(
              BeliefFormulaType::FALSE_FORMULA);
...
BeliefFormula PlankFormulaConverter::convert_not(
  ...
  result.set_bf1(
      convert(formula->get_formula()));

  return result;
}

BeliefFormula PlankFormulaConverter::convert_nary(
...
AgentsSet PlankFormulaConverter::convert_agents(
  ...
  for (const plank::del::agent plank_agent :
       plank_agents) {
...
BeliefFormula PlankFormulaConverter::convert_imply(
    ...
    negated_antecedent.set_formula_type(
        BeliefFormulaType::PROPOSITIONAL_FORMULA);
    negated_antecedent.set_operator(
        BeliefFormulaOperator::BF_NOT);

    negated_antecedent.set_bf1(
        convert(formula->get_first_formula()));
    ...
    result.set_bf2(
        convert(formula->get_second_formula()));

    return result;
}

BeliefFormula PlankFormulaConverter::make_not(
...
BeliefFormula PlankFormulaConverter::make_or(
...
BeliefFormula PlankFormulaConverter::convert_diamond(
    ...
    const AgentsSet agents =
        convert_agents(formula->get_mod_index());
    ...
    const BeliefFormula inner =
        convert(formula->get_formula());

    const BeliefFormula negated_inner =
        make_not(inner);

    BeliefFormula box;

    if (agents.size() == 1) {
        box.set_formula_type(
            BeliefFormulaType::BELIEF_FORMULA);
...
BeliefFormula PlankFormulaConverter::convert_kw_box(
    ...
    if (agents.size() == 1) {
        ...
        knows_true.set_formula_type(
            BeliefFormulaType::BELIEF_FORMULA);

        knows_true.set_agent(agent);
        knows_true.set_bf1(inner);

        knows_false.set_formula_type(
            BeliefFormulaType::BELIEF_FORMULA);
...
BeliefFormula PlankFormulaConverter::convert_kw_diamond(
    ...
    const BeliefFormula double_negated_inner =
        make_not(negated_inner);
