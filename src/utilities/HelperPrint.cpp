...
void HelperPrint::print_dot_format(const KripkeState &kstate, std::ostream &os) const {
    ...
    for (const auto &set: for_rank_print | std::views::values) {
        ...
        for (const auto &world_ptr: set) {
            os << "\"" << map_rep_to_name[world_ptr.get_repetition()] << "_" << map_world_to_index[world_ptr.
                get_fluent_set()] << "\"; ";
        }
        os << "}\n";
    }
    ...
    for (const auto &world_ptr: worlds) {
        const auto &tmp_fs = world_ptr.get_fluent_set();
        bool print_first = false;
        os << "		<tr><td>" << map_rep_to_name[world_ptr.get_repetition()] << "_" << map_world_to_index[tmp_fs] <<
                "</td> <td>";
...
void HelperPrint::print_dataset_format(const KripkeState &kstate, std::ostream &os) const {
    ...
    /*for (const auto& [hash, id] : world_map) {
        os << (use_hash ? std::to_string(hash) : adjust_id_wrt_agents(id)) << ";" << std::endl;
    }*/

    // Pointed world
    const auto pointed_hash = kstate.get_pointed().get_id();
    os << (use_hash ? std::to_string(pointed_hash) : adjust_id_wrt_agents(world_map[pointed_hash]))
    ...
    /*     for (const auto& [edge, agents] : edge_map) {
            ...
            for (const auto& ag : agents) {
                if (!first) os << ",";
                os << domain::get_instance().get_grounder().deground_agent(ag);
                first = false;
            }
            os << "\"];" << std::endl;
        } */
