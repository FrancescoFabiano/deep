 ...
 *
 * An ActionLevel represents a set of executable actions at a specific depth in the planning graph.
 *
...
public:
    ...
    /**
     * \brief Default constructor. Initializes the action level with depth 0 and an empty action set.
     */
    ActionLevel() = default;
    ...
    /**
     * \brief Constructs an ActionLevel with a given set of actions and a specific depth.
     * \param[in] actions The set of actions to assign to this level.
     ...
     */
    [[nodiscard]] ActionLevel& operator=(const ActionLevel& to_assign);
