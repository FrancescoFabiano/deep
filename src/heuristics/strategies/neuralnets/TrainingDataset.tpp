...
bool TrainingDataset<StateRepr>::dfs_exploration(
    ...
    return !global_dataset.empty() &&
           (m_goal_founds > 0); // Return true if dataset is not empty and goals
    // were found
}
...
int TrainingDataset<StateRepr>::dfs_worker(
    ...
    // Initial score (goal = 0, otherwise failed)
        ...
        for (const auto &action : local_actions) {
            if (state.is_executable(action)) {
                ...
                if (m_total_possible_nodes_log > m_threshold_node_generation_log) {
                    ...
                    const double fullness_ratio = static_cast<double>(m_current_nodes) /
                    ...
                    discard_probability += std::min(
                        0.01 * std::pow(static_cast<double>(m_discard_augmentation_factor) / (3 * max_depth), 2),
                        0.1
                    );
                    ...
                    const auto discard_factor = ArgumentParser::get_instance().get_dataset_discard_factor();
    ...
    // Only update if a valid successor score was found
    if (best_successor_score != m_failed_state &&
