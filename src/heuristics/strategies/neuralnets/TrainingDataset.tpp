...
template <StateRepresentation StateRepr>
constexpr auto TrainingDataset<StateRepr>::get_to_goal_edge_id() -> const std::string&
{
    return m_to_goal_edge_id;
}

template <StateRepresentation StateRepr>
constexpr const std::string& TrainingDataset<StateRepr>::get_to_state_edge_id()
{
    return m_to_state_edge_id;
}

template <StateRepresentation StateRepr>
constexpr const std::string& TrainingDataset<StateRepr>::get_epsilon_node_id()
{
    return m_epsilon_node_id;
}

template <StateRepresentation StateRepr>
constexpr const std::string& TrainingDataset<StateRepr>::get_goal_parent_id()
{
    return m_goal_parent_id;
}

template <StateRepresentation StateRepr>
constexpr const std::string& TrainingDataset<StateRepr>::get_goal_string() const
{
...
TrainingDataset<StateRepr>::TrainingDataset()
{
    ...
    // We will also generate the goal edges and shift for them as well. Done in goal generation
    ...
    print_goal_tree(); // Only needed if we do not use the goal and state merged together
}
...
bool TrainingDataset<StateRepr>::generate_dataset()
{
    ...
    result << "Path Hash,Path Hash Merged,Path Mapped,Path Mapped Merged,Depth,Distance From Goal,Goal" << std::endl;
    result.close();
...
void TrainingDataset<StateRepr>::generate_goal_tree_subgraph()
{
    ...
    // The final value of shits so that state, when mapped starts from the latest node generated for the goals + 1
...
template <StateRepresentation StateRepr>
void TrainingDataset<StateRepr>::generate_goal_subtree(const BeliefFormula& to_print, const size_t goal_counter,
                                                       size_t& next_id,
    ...
    case BeliefFormulaType::BELIEF_FORMULA:
        {
            ...
            generate_goal_subtree(to_print.get_bf1(), goal_counter, next_id, node_name, os);
            break;
        }
    ...
    case BeliefFormulaType::PROPOSITIONAL_FORMULA:
        {
            ...
            {
                generate_goal_subtree(to_print.get_bf2(), goal_counter, next_id, node_name, os);
            }
...
std::string TrainingDataset<StateRepr>::format_row(const State<StateRepr>& state, const size_t depth, const int score)
{
    ...
    std::string filename_hash = format_name(base_filename, OutputPaths::DATASET_NN_DATASET_HASHED, false);
    std::string filename_hash_merged = format_name(base_filename, OutputPaths::DATASET_NN_DATASET_HASHED, true);
    std::string filename_emap = format_name(base_filename, OutputPaths::DATASET_NN_DATASET_MAPPED, false);
    std::string filename_emap_merged = format_name(base_filename, OutputPaths::DATASET_NN_DATASET_HASHED, true);
    ...
    ss << filename_hash << "," << filename_hash_merged << "," << filename_emap << "," << filename_emap_merged << "," <<
        depth << "," << score << "," << m_goal_file_path;
...
std::string TrainingDataset<StateRepr>::print_state_for_dataset(const State<StateRepr>& state)
{
    ...
    if (ArgumentParser::get_instance().get_dataset_mapped() || ArgumentParser::get_instance().get_dataset_both())
    {
        if (ArgumentParser::get_instance().get_dataset_merged() || ArgumentParser::get_instance().
            get_dataset_merged_both())
        {
            print_state_for_dataset_internal(state, base_filename, OutputPaths::DATASET_NN_DATASET_MAPPED, true);
        }
        if (!ArgumentParser::get_instance().get_dataset_merged() || ArgumentParser::get_instance().
            get_dataset_merged_both())
        {
            print_state_for_dataset_internal(state, base_filename, OutputPaths::DATASET_NN_DATASET_MAPPED, false);
        }
    }
    if (!ArgumentParser::get_instance().get_dataset_mapped() || ArgumentParser::get_instance().get_dataset_both())
    {
        if (ArgumentParser::get_instance().get_dataset_merged() || ArgumentParser::get_instance().
            get_dataset_merged_both())
        {
            print_state_for_dataset_internal(state, base_filename, OutputPaths::DATASET_NN_DATASET_HASHED, true);
        }
        if (!ArgumentParser::get_instance().get_dataset_merged() || ArgumentParser::get_instance().
            get_dataset_merged_both())
        {
            print_state_for_dataset_internal(state, base_filename, OutputPaths::DATASET_NN_DATASET_HASHED, false);
        }
    }
                                                                  ...
                                                                  const bool merged) const
{
    ...
    state.print_dataset_format(out, type == OutputPaths::DATASET_NN_DATASET_HASHED, merged);
    out.close();
}

template <StateRepresentation StateRepr>
std::string TrainingDataset<StateRepr>::format_name(const std::string& base_filename, const std::string& type,
                                                    const bool merged) const
{
    const std::string merge_folder = merged
                                         ? OutputPaths::DATASET_NN_DATASET_MERGED
                                         : OutputPaths::DATASET_NN_DATASET_SEPARATED;
    std::string result_filename = m_training_raw_files_folder + type + "/" + merge_folder + "/" + base_filename + "_" +
        type + ".dot";
