...
private:
    ...
    /// \brief Integer node ID that connects the two sub-graphs (goal and state) in the merged graph.
    static constexpr int m_epsilon_node_id_int = 0;
    /// \brief String node ID that connects the two sub-graphs (goal and state) in the merged graph.
