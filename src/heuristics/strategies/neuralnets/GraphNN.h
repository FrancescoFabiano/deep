...
public:
  ...
  /**
   * \brief Get the score for a given state using the neural network heuristic using native C++ code
  ...
  /**
   * \brief Get the score for a given state using the neural network heuristic thorugh the python code
   * \tparam StateRepr The state representation type.
   * \param state The state to evaluate.
   * \return The heuristic score for the state.
   */
...
private:
  ...
  GraphTensor m_goal_graph_tensor; ///< This is the goal tensor, computed only once for efficiency. If merged is active also the additional structural nodes are added

  size_t m_symbolic_id = 0; ///< Current symbolic ID counter (will be incremented if a new ID is assigned)
  std::unordered_map<size_t, size_t> m_node_to_symbolic; ///<  Map from real node IDs to symbolic IDs.

  std::vector<int64_t> m_real_node_ids; ///< Vector storing real node IDs in symbolic order. (Assume that the position is meaningful)
  ...
  size_t m_edges_initial_size = 0;      ///< Initial size for the edges vector (to remove the new inserted nodes while processing the heuristics)
  size_t m_node_ids_initial_size = 0;   ///< Initial size for the node IDs vector (to remove the new inserted nodes while processing the heuristics)
  size_t m_starting_symbolic_id = 0;    ///< Initial symbolic ID (to remove the new inserted nodes while processing the heuristics)
  ...
  GraphTensor state_to_tensor_minimal(const KripkeState &kstate);
    ...
    /**
    * \brief Converts the goal graph into the info that will then be added to a Tensor.
    * If the tensor are generated with the goal (merged) this info will be directly embedded in the states.
    * Otherwise, it will populate m_goal_graph_tensor to be passed as argument.
    */
    void populate_with_goal();

    /**
     * \brief Returns the symbolic ID for a node, assigning a new one if it does not exist.
     *
     * If the given node is already present in the m_node_to_symbolic map, returns its symbolic ID.
     * Otherwise, assigns the next available symbolic ID to the node, updates the map,
     * appends the real node ID to the m_real_node_ids vector, and increments the m_symbolic_id counter.
     *
    ...
    /**
     * \brief Adds an edge to the graph representation while also adding the symbolic ids of the two vertices.
     *
