...
GraphNN<StateRepr>::get_score(const State<StateRepr> &state) {
    ...
    //We use both to indicate that we want to use not merged with no goal -- just debuging phase
    // \todo adjust this
    ...
    const auto result =
            static_cast<short>(std::round(inference_result * 1000));
...
template<StateRepresentation StateRepr>
float GraphNN<StateRepr>::run_inference_with_goal(const GraphTensor &tensor) const {
    if (!m_model_loaded) {
        ExitHandler::exit_with_message(ExitHandler::ExitCode::GNNInstanceError,
                                       "[ONNX] Model not loaded before inference.");
    }
    ...
    std::vector<float> goal_node_ids_float(goal.real_node_ids.begin(), goal.real_node_ids.end());
    const std::array<int64_t, 1> goal_node_ids_shape{static_cast<int64_t>(goal_node_ids_float.size())};
    ...
    const std::array<int64_t, 2> goal_edge_index_shape{2, static_cast<int64_t>(num_goal_edges)};
    ...
    std::vector<float> goal_edge_attrs_float(goal.edge_attrs.begin(), goal.edge_attrs.end());
    const std::array<int64_t, 2> goal_edge_attr_shape{static_cast<int64_t>(num_goal_edges), 1};
    ...
    const std::array<int64_t, 1> goal_state_batch_shape{static_cast<int64_t>(goal_state_batch_data.size())};
