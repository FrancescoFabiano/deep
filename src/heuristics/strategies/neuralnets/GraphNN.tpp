...
GraphNN<StateRepr>::state_to_tensor_minimal(const KripkeState &kstate) {
  ...
  for (const auto &state_parent : kstate.get_designated_worlds()) {

    const auto state_parent_id =
        static_cast<int64_t>(world_map.at(state_parent.get_id_casted()));

    m_pointed_ids.push_back(get_symbolic_id(state_parent_id, state_parent));
  ...
  for (const auto &[from_pw, from_map] : kstate.get_beliefs()) {
    ...
    const int64_t src =
        static_cast<int64_t>(world_map.at(from_pw.get_id_casted()));
    ...
    for (const auto &[agent, to_set] : from_map) {
      ...
      for (const auto &to_pw : to_set) {
        ...
        const int64_t dst =
            static_cast<int64_t>(world_map.at(to_pw.get_id_casted()));
...
bool GraphNN<StateRepr>::check_tensor_against_dot(
  ...
  if (!ArgumentParser::get_instance().get_dataset_separated()) {
    ...
    for (size_t e = 0; e < state_tensor.edge_attrs.size(); ++e) {

      if (state_tensor.edge_attrs[e] == to_state_label) {

        expected_pointed_ids.push_back(state_tensor.edge_dst[e]);
      }
    }

    if (state_tensor.pointed_ids != expected_pointed_ids) {
