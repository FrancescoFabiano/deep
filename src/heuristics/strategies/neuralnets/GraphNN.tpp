    ...
    if (ArgumentParser::get_instance().get_verbose())
    {
        if (!check_tensor_against_dot(state_tensor, state))
        {
            ExitHandler::exit_with_message(
                ...
                "[ERROR] Error while comparing the state in " + m_checking_file_path + ". Its tensor representation generated a different dot file.");
        }
        else
        {
            ArgumentParser::get_instance().get_output_stream() << "[DEBUG] State represented as dot and as tensor match:)" << std::endl;
        }
    }
#endif
...
GraphNN<StateRepr>::populate_with_goal()
{
    ...
    {
        constexpr auto epsilon_id = TrainingDataset<KripkeState>::get_epsilon_node_id_int();
        constexpr auto goal_parent_id = TrainingDataset<KripkeState>::get_goal_parent_id_int();
    ...
    if (!ArgumentParser::get_instance().get_dataset_merged())
    {
        ...
        const torch::Tensor edge_ids = torch::stack({
            torch::from_blob(m_edge_src.data(), {static_cast<int64_t>(m_edge_src.size())}, m_options),
            torch::from_blob(m_edge_dst.data(), {static_cast<int64_t>(m_edge_dst.size())}, m_options)
        });

        const torch::Tensor edge_attrs = torch::from_blob(m_edge_labels.data(),
                                                          {static_cast<int64_t>(m_edge_labels.size()), 1}, m_options);
...
template <StateRepresentation StateRepr>
bool GraphNN<StateRepr>::check_tensor_against_dot(const GraphTensor& state_tensor, const State<StateRepr>& state) const
{
    ...
    for (int64_t e = 0; e < num_edges; ++e) {
        ...
        if (edge_attrs.defined() && edge_attrs.numel() > e) {
            int64_t attr = edge_attrs[e][0].item<int64_t>(); // assuming shape [num_edges,1]
    ...
    while (true)
    {
        ...
        if (file1_eof != file2_eof) {
            os << "Files differ in length at line " << line_number << "." << std::endl;
            return false;
        }
        ...
        if (line1 != line2) {
            ...
            size_t col = 0;
            while (col < line1.size() && col < line2.size() && line1[col] == line2[col]) {
                ++col;
            }
