...
void ArgumentParser::parse(int argc, char** argv)
{
    ...
    try
    {
        ...
        if (!m_dataset_mode &&
            (app.count("--dataset_depth") || app.count("--dataset_mapped") ||
        ...
        if (m_search_strategy == "HFS" && m_heuristic_opt != "GNN" &&
            app.count("--GNN_model"))
        {
            ExitHandler::exit_with_message(ExitHandler::ExitCode::ArgParseError,
                                           "--GNN_model can only be used with "
                                           "--search HFS and --heuristics GNN.");
        }
...
ArgumentParser::ArgumentParser() : app("deep")
{
    ...
    search_group
        ->add_option("-s,--search", m_search_strategy,
                     ...
                     "Depth First Search), or 'HFS' (Heuristic First Search).")
    ...
    search_group
        ->add_option(
            ...
            "If GNN is enabled, ensure you are using a model compiled with the 'ENABLE_NEURALNETS' option; otherwise, torch will not be installed or linked for efficiency purposes.")
