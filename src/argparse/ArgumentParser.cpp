...
void ArgumentParser::parse(int argc, char** argv)
{
    ...
    try
    {
        ...
        if (m_search_strategy != "HFS" && app.count("--heuristics"))
        {
            ExitHandler::exit_with_message(
                ExitHandler::ExitCode::ArgParseError,
                "--heuristics can only be used with --search HFS."
            );
        }
        if (m_search_strategy == "HFS" && m_heuristic_opt != "GNN" && app.count("--GNN_model"))
        {
            ExitHandler::exit_with_message(
                ExitHandler::ExitCode::ArgParseError,
                "--GNN_model can only be used with --search HFS and --heuristics GNN."
            );
        }
...
ArgumentParser::ArgumentParser() : app("deep")
{
    app.add_option("input_file", m_input_file,
                   "Specify the input problem file (e.g., problem.epddl). This file defines the planning problem.")
       ->required();
    ...
    dataset_group->add_option("--dataset_depth", m_dataset_depth,
                 ...
                 ->default_val("10");
    dataset_group->add_flag("--dataset_mapped", m_dataset_mapped,
                            "Use mapped (compact) node labels in dataset generation. If not set, hashed node labels are used.");
    dataset_group->add_flag("--dataset_both", m_dataset_both,
                            "Generate both mapped and hashed node labels in the dataset.");
    dataset_group->add_flag("--dataset_merged", m_dataset_merged, "Enable merged dataset generation mode.");
    dataset_group->add_flag("--dataset_merged_both", m_dataset_merged_both,
...
const std::string& ArgumentParser::get_GNN_model_path() const noexcept { return m_GNN_model_path; }
