...
void KripkeState::build_initial(std::ostream& os) {
...
void KripkeState::maintain_oblivious_believed_worlds(KripkeState& ret, const AgentsSet& oblivious_obs_agents) const {
    if (!oblivious_obs_agents.empty()) {
        auto tmp_world_set = KripkeReachabilityHelper::get_E_reachable_worlds(oblivious_obs_agents, get_pointed(), *this);
        KripkeWorldPointersSet world_oblivious;
        for (const auto& agent : Domain::get_instance().get_agents()) {
            for (const auto& wo_ob : tmp_world_set) {
                SetHelper::sum_set<KripkeWorldPointer>(world_oblivious, KripkeReachabilityHelper::get_B_reachable_worlds(agent, wo_ob, *this));
            }
        }
...
KripkeState KripkeState::execute_sensing(const Action& act) const {
    ...
    KripkeWorldPointer new_pointed = execute_sensing_announcement_helper(
        effects, ret, get_pointed(), calculated, partially_obs_agents, oblivious_obs_agents, KripkeEntailmentHelper::entails(effects, get_pointed()));
    ret.set_pointed(new_pointed);
