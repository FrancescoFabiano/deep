# Experiment Batch 3: Scalability Testing
...
``` console
python3 scripts/gnn_exp/create_all_training_data.py exp/gnn_exp/batch3 --deep_exe cmake-build-release-nn/bin/deep
```
...
``` console
python3 scripts/gnn_exp/create_all_training_data.py exp/gnn_exp/batch3 --deep_exe cmake-build-release-nn/bin/deep --depth 40 --discard_factor 0.2
```
...
``` console
python3 scripts/gnn_exp/create_all_training_data.py exp/gnn_exp/batch3 --deep_exe cmake-build-release-nn/bin/deep --dataset_type BITMASK
```
...
``` console
python3 scripts/gnn_exp/train_models.py exp/gnn_exp/batch3
```
...
```console
python3 scripts/gnn_exp/train_models.py exp/gnn_exp/batch3 --dataset_type BITMASK
```
...
``` console
python3 scripts/gnn_exp/bulk_coverage_run.py cmake-build-release-nn/bin/deep exp/gnn_exp/batch3/ --threads 8 --binary_args "-s Astar -u GNN -c -b" --timeout 600
```

Example with changed dataset type:

``` console
python3 scripts/gnn_exp/bulk_coverage_run.py cmake-build-release-nn/bin/deep exp/gnn_exp/batch3/ --threads 8 --binary_args "-s Astar -u GNN -c -b --dataset_type BITMASK" --timeout 600
```
...
``` console
python3 scripts/gnn_exp/bulk_coverage_run.py cmake-build-release-nn/bin/deep exp/gnn_exp/batch3/ --threads 8 --binary_args "-c -b" --timeout 600
```
...
- the path to the deep executable (`cmake-build-release-nn/bin/deep` in
  the example)
- the path to the experiment folder (`exp/gnn_exp/batch3/` which
  is this folder)
