...
``` console
python3 scripts/gnn_exp/create_all_training_data.py exp/gnn_exp/batch4 --deep_exe cmake-build-release-nn/bin/deep --dataset_type BITMASK
```
...
```console
python3 scripts/gnn_exp/train_models.py exp/gnn_exp/batch4 --dataset_type BITMASK
```
...
``` console
python3 scripts/gnn_exp/bulk_coverage_run.py cmake-build-release-nn/bin/deep exp/gnn_exp/batch4/ --threads 8 --binary_args "-s Astar -u GNN -c -b" --timeout 600
```

Example with changed dataset type:

``` console
python3 scripts/gnn_exp/bulk_coverage_run.py cmake-build-release-nn/bin/deep exp/gnn_exp/batch4/ --threads 8 --binary_args "-s Astar -u GNN -c -b --dataset_type BITMASK" --timeout 600
```
...
- the path to the deep executable (`cmake-build-release-nn/bin/deep` in
  the example)
- the path to the experiment folder (`exp/gnn_exp/batch4/` which
  is this folder)
